...
*/
int cont = 1;
int dfs(int elemento) {
    visitados[elemento] = 1;
    for (int i = 0; i < grafo[elemento].size(); i++)
    {
        if (visitados[grafo[elemento][i]] == 0) {
            cont++;
            dfs(grafo[elemento][i]);
...
int main() {
    ...
    while(cin >> espc >> info){
    grafo.resize(espc);
    visitados.resize(espc, 0);
    for (int j = 0; j < info; j++) {
        int espcA, espcB;
        cin >> espcA;
        cin >> espcB;
        grafo[espcA - 1].push_back(espcB - 1);
        grafo[espcB - 1].push_back(espcA - 1);
    }
    int elemento;
    cin >> elemento;
    int res = dfs(elemento-1);
    cout << res << endl;
    cont = 1;
    grafo.clear();
    visitados.clear();
