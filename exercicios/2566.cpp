...
int main() {
    ...
    cout.tie(0);
    int cidades, trechos;
    while (cin >> cidades >> trechos) {

    Graph grafo1(cidades);
    Graph grafo2(cidades);
    for (int j = 0; j < trechos; j++) {
        int conA, conB, meio,reais;
        cin >> conA >> conB >> meio>> reais;
        if (meio == 0) {

        grafo1.addEdge(conA - 1, conB - 1, reais);
        }
        else if (meio == 1) {
            grafo2.addEdge(conA - 1, conB - 1, reais);
        }
    }
    vector<double> valor2 = grafo2.Dijkstra(0);
    vector<double> valor1 = grafo1.Dijkstra(0);
    if (valor2[valor2.size() - 1] < valor1[valor1.size() - 1]) {

        cout << valor2[valor2.size()-1] << endl;
    }
    else {

        cout << valor1[valor1.size()-1] << endl;
    }
    valor1.clear();
    valor2.clear();
    }
